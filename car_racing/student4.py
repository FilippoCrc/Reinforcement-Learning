...
# PrioritizedReplayBuffer remains the same as it's agnostic to action type
class PrioritizedReplayBuffer:
    ...
    def sample(self, batch_size, beta=0.4):
        ...
        # CHANGE: actions are now integers instead of continuous values
...
class DDQNNet(nn.Module):
    # CHANGE: num_actions now represents discrete action space size
    ...
    def forward(self, x):
        ...
        x = F.relu(self.fc1(x))
        return self.fc2(x)  # No more tanh, outputs raw Q-values
...
class Policy:
    ...
    def act(self, state, training=False):
        ...
        return action_idx  # Return discrete index
